...
class DefaultBuilder:
    ...
    def T1(self, T: float, frequency: float) -> float:
        ...
        logging.info(f"y0 is currently {y0}")
    ...
    def T2(self, T: float, frequency: float) -> float:
        ...
        logging.info(f"gamma_psi_base is currently {gamma_psi_base}")
    ...
    def calculate_parameter_error(self, T: float, qb_path: str, type="ratio", parameter="T1", override_config: dict = None):
        if override_config is not None:
            ...
            logging.info(f"here is the whole config before overriding: {self.config}")
            for key, val in override_config.items():
                self.config[key] = val
            logging.info(f"here is the whole config after overriding: {self.config}")
            self.is_valid()
        ...
        logging.info(f"actual_{parameter}: {actual_parameter}\n")
        logging.info(f"calculating {parameter} for the initial temperature of {T} now\n")
        if parameter == "T1":
        ...
        logging.info(f"calculated_{parameter}: {calculated_parameter}")
        if type == "ratio":
    ...
    def calculate_qb_config(self, control_parameters: dict, qb_path: str):
        ...
        logging.info(f"calculating T1 for the working temperature of {T} now\n")
    ...
    def optimise_parameters(self):
        ...
        logging.info("\nNow optimising y0")
        optimiser_y0 = lambda cur_y0: self.total_init_parameter_error({"y0": cur_y0}, "T1")
        ...
        optimiser_y_phi_b = lambda cur_y_phi_b: self.total_init_parameter_error({"gamma_psi_base": cur_y_phi_b}, "T2")
